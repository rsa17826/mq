...
const RoomGraph = (function () {
  ...
  function computeRoomComponents(room, have) {
    ...
    if (!room.areas || room.areas.length === 0) {
      ...
      for (let i = 1; i < allKeys.length; i++) union(allKeys[0], allKeys[i]);
    } else {
  ...
  function externalConnection(roomKey, room, side, idx) {
    ...
    if (typeof ER_MAP !== "undefined" && ER_MAP && typeof ER_MAP.get === "function") {
      const conns = (ER_MAP.get(roomKey) || []).filter((c) => c.origSide === side);
      if (conns.length > 0) {
        const exact = conns.find((c) => String(c.origIdx) === String(idx));
        const conn = exact || conns[0];
        return { room: `${conn.newNorth}_${conn.newEast}`, side: conn.exitSide, idx: Number(conn.exitIdx) };
      }
    }
  ...
  function computeReachability(haveReal, startRoom) {
    ...
    function totalExitsFor(room) {
      let n = 0;
      for (const side of Object.keys(room.exits || {})) n += room.exits[side].length;
      return n;
    }
    ...
    if (roomIndex[startRoom]) {
      ...
      for (const key of Object.keys(comps)) {
        const [side, idxStr] = [key.replace(/\d+$/, ""), key.match(/\d+$/)[0]];
    ...
    while (queue.length > 0) {
      ...
      const dest = externalConnection(cur.room, room, cur.side, cur.idx);
      if (dest && roomIndex[dest.room]) {
        const destRoom = roomIndex[dest.room];
        if (!roomExitCounts[dest.room]) {
          roomExitCounts[dest.room] = { reachable: 0, total: totalExitsFor(destRoom) };
        }
        ...
        if (!reachableExits.has(destNode)) {
          ...
          queue.push({ room: dest.room, side: dest.side, idx: dest.idx });
        }
    ...
    function roomStatus(room) {
      ...
      if (!counts || counts.total === 0) return visitedRooms.has(room) ? "full" : "none";
      if (counts.reachable === 0) return "none";
      if (counts.reachable >= counts.total) return "full";
      return "partial";
    }

    return { reachableExits, roomExitCounts, isEntranceReachable, roomStatus, visitedRooms };
  }
