...
def init(src):
    ...
    ;(function () {{
      ...
      window.setManagerData = function (k, v, vv) {{
        ...
        if (vv === undefined) {{
          ...
          var currentValue = manager[k][v] || 0
        }}
        if (vv!==undefined?typeof vv === 'number':typeof v === 'number'){{
          ...
          if (isGain) {{
            console.log("[IR GAIN] '" + (vv?k+' '+v:k) + "': " + currentValue + " → " + (vv??v) + " at room " + playerPos.room)
            ...
            if (now - lastCollectTime > 100) {{  // 100ms debounce
              ...
              if (closestItem) {{
                ...
                console.log("[IR INFO] No matching randomized item location found for '" + (vv?k+' '+v:k) + "'")
              }}
              IR_COLLECTION_TIMEOUT.set(k, now)
            }}else{{
              console.log("[IR INFO] SKIPPED '" + (vv?k+' '+v:k) + "': " + currentValue + " → " + (vv??v))
            }}
          }} else if (isLoss) {{
            console.log("[IR LOSS] '" + (vv?k+' '+v:k) + "': " + currentValue + " → " + (vv??v))
          }}
        }}else{{
          console.log("[IR INFO]'",k,v,vv)
        }}
  ...
  print(f"[ADVANCED] Patched item randomizer")
